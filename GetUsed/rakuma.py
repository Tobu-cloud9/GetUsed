...
class Rakuma:
    ...
    def scraping(self, search, keyword, min_price, max_price, category, status, quality):
        ...
        num = 1
        while num < 251:
            url = "https://fril.jp/s?query=" + keyword + category_dict[category] + "&min=" + str(min_price) + "&max=" + str(max_price) + status_dict[status] + quality
            ...
            for link_db, name_db, price_db, image_db in zip(link, name, price, image):
                Item.objects.bulk_create([
                    Item(item_search=search, item_type='R', item_category=category, item_status=status, item_link=link_db, item_name=name_db, item_price=price_db, item_image=image_db)
                ])
            num += 50